...
export default function QuizPage() {
  ...
  // If no topicId, show quiz selector
  if (!topicId) {
    return <QuizSelector />
  }
    ...
    <AppShell>
      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          ...
          <p className="text-gray-500 mt-1">Test your knowledge on each topic. Complete all lessons first!</p>
        </div>
...
function QuizRunner({ quiz, topic, submitQuiz }) {
  ...
  const handleNext = () => {
    if (isLast) {
      const newScore = answers.filter(a => a === 'correct').length + (selected && quiz.questions[current].options.find(o => o.id === selected)?.isCorrect ? 1 : 0)
      // score is already updated via setScore
      const finalScore = score + (quiz.questions[current].options.find(o => o.id === selected)?.isCorrect ? 0 : 0)
    ...
    <AppShell>
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to={`/topics/${topic.id}`} className="text-sm text-gray-400 hover:text-gray-600">
            ✕ Exit
          </Link>
        </div>
