...
export default function LessonPage() {
  ...
  const handleComplete = () => {
    if (!isDone) {
      ...
      const allDone = topic.lessons.every(
        l => l.id === lessonId || !!completedLessons[l.id]
      )
      if (allDone) completeTopic(topicId)
    }
    if (nextLesson) {
      navigate(`/topics/${topicId}/lessons/${nextLesson.id}`)
    } else {
    ...
    <AppShell>
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-400 mb-4 flex-wrap">
          ...
          <span>→</span>
          <Link to={`/topics/${topicId}`} className="hover:text-gray-600">{topic.title}</Link>
          <span>→</span>
